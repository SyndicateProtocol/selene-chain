...
export default function HowTo() {
  ...
  const transactionInfo = useMemo<Record<TransactionType, TransactionInfo>>(
    () => ({
      lowCalldata: {
        description:
          "Efficient transactions with minimal calldata (≤ 100 bytes), optimizing for lower costs.",
        code: `
const hash = await walletClient.writeContract({
  ...
  args: ['0x1234'] 
      ...
      contractCall: {
        description:
          "Transactions targeting only the allowed Waxing Crescent contract call.",
        code: `
const hash = await walletClient.writeContract({
  address: moonPhaseContract.address,
  abi: moonPhaseAbi,
  ...
  args: []
      ...
      angelNumber: {
        description:
          "Transactions with angel numbers as ETH values (e.g., 0.333 ETH, 0.555 ETH).",
        code: `
const hash = await walletClient.writeContract({
  address: moonPhaseContract.address,
  abi: moonPhaseAbi,
  ...
  value: BigInt(111), // Angel number: 111 wei
      ...
      waxingGibbous: {
        description:
          "Transactions that specifically call the waxingGibbous() function.",
        code: `
const hash = await walletClient.writeContract({
  address: moonPhaseContract.address,
  abi: moonPhaseAbi,
  ...
  args: [],
      ...
      tokenTransfer: {
        description:
          "Transactions interacting with ERC20, ERC721, or ERC1155 token contracts.",
        code: `
const hash = await walletClient.writeContract({
...
const hash = await walletClient.writeContract({
  ...
  gas: BigInt(100000)  // Gas limit ≥ 21,000
      ...
      balancedGas: {
        description:
          "Balanced transactions with gas limit ≥ 1,000,000 and calldata ≤ 1,000 bytes.",
        code: `
const hash = await walletClient.writeContract({
  address: moonPhaseContract.address,
  abi: moonPhaseAbi,
  ...
  const handleRunTransaction = useCallback(
    async (type: TransactionType) => {
      ...
      try {
        ...
        await sendTransaction(config.functionSignature, config.args, {
          value: config.value
        })
            ...
            <AccordionItem key={type} value={type}>
              ...
              <AccordionContent>
                <p className="text-sm mb-3">
                  {transactionInfo[type].description}
                </p>
